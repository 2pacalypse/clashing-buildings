...
- **Scalability for large datasets:**
  ...
  - 3D R-tree spatial join: Use a spatial index (such as a 3D R-tree) to efficiently filter candidate building pairs whose bounding boxes overlap. This reduces the number of expensive geometry checks, but the actual intersection tests are still performed on the CPU.
  - GPU-accelerated spatial join: Use massively parallel GPU kernels to perform intersection checks on candidate pairs (which may be filtered by a spatial index like an R-tree). This approach leverages GPU hardware for high throughput and is especially effective for very large datasets. Libraries such as RAPIDS cuSpatial or custom CUDA implementations can be used for this purpose.
