...
export function renderGeoJSON(geojson, overlaps = null) {
    clearScene();
...
export function loadSample(sampleKey) {
    const sample = samples[sampleKey];
    
    // Toggle selection
    if (selectedSamples.has(sampleKey)) {
        selectedSamples.delete(sampleKey);
        try { document.querySelector(`[data-sample="${sampleKey}"]`).classList.remove('active'); } catch (e) {}
    } else {
        selectedSamples.add(sampleKey);
        try { document.querySelector(`[data-sample="${sampleKey}"]`).classList.add('active'); } catch (e) {}
    }
...
export function renderAllSelectedSamples() {
    clearScene();
